fun assembleCandidateBlock(): Block {
  ...
  block.transactions.addFirst(createCoinbaseTransacion())
  for(txId in includedTransactions) {
    block.transactions.addLast(createTransaction(execBitcoinCommand(GET_TRANSACTION + " " + txId)))
  }
  return block
}

fun chooseNumberOfTransactions(txIds: List<String>): Int {
  //TODO how many TX?
  return 511
}

fun createCoinbaseTransacion(): Transaction {
...
fun createTransaction(rawTxData: String) : Transaction {
  //TODO Implement construction of MemPool TX
  return Transaction("BLABLABLA")
}

fun retrieveTransactionsIds(): List<String> {
